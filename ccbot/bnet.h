...
private:
	...
	string m_FirstChannel;						// the first chat channel to join upon entering chat 
...
public:
	...
	vector<CUser *> m_Channel;					// map of users in channel
	...
	void SendChatCommand( const string &chatCommand, bool console );
	void SendChatCommandHidden( const string &chatCommand, bool console );
	void SendGetClanList( );
	void SendClanChangeRank( string accountName, CBNETProtocol::RankCode rank );
	
	// other functions

	void QueueChatCommand( const string &chatCommand, bool console );
	void QueueChatCommand( const string &chatCommand, string user, bool whisper, bool console );
	void QueueWhisperCommand( const string &chatCommand, string user, bool console );
	void ImmediateChatCommand( const string &chatCommand, bool console );
	void ImmediateChatCommand( const string &chatCommand, string user, bool whisper, bool console );
...
private:
	string m_User;
	string m_LowerUser;							// lowercase user
	int m_Ping;
	uint32_t m_UserFlags;
	string m_Clan;
public:
	CUser( const string &nUser, const string &nLowerUser, int nPing, uint32_t nUserFlags );
	~CUser( );

	string GetUser( )							{ return m_User; }
	string GetLowerUser( )							{ return m_LowerUser; }
